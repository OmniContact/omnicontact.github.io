...
export class DragStateManager {
    ...
    update() {
        if (this.worldHit && this.localHit && this.currentWorld && this.arrow && this.physicsObject) {
            ...
            this._tmpOffset.copy(this.currentWorld).sub(this.worldHit);
            this.offset = this._tmpOffset;
            this._tmpDir.copy(this._tmpOffset).normalize();
            this.arrow.setDirection(this._tmpDir);
            this.arrow.setLength(this._tmpOffset.length());
        }
    }
    end(evt) {
