...
import { InteractionPolicyRunner } from './interactionPolicyRunner.js';
import { CarryBoxWTACPolicyRunner } from './carryBoxWTACPolicyRunner.js';
...
export async function reloadScene(mjcf_path) {
  // Dispose old scene resources (geometries, materials, textures) before removing
  ...
  for (let b = 0; b < this.model.nbody; b++) {
    ...
    const bodyName = textDecoder.decode(bodyNamesArray.subarray(start_idx, end_idx));
    this.bodyNameToId[bodyName] = b;
  ...
  // reads the robot's root state instead of the first body's (which may be
  // a scene object with its own freejoint).
  this.rootQposAdr = 0;
  this.rootQvelAdr = 0;
  if (this.pelvis_body_id !== undefined) {
    const jntAdr = this.model.body_jntadr[this.pelvis_body_id];
    const jntNum = this.model.body_jntnum[this.pelvis_body_id];
    if (jntNum > 0 && jntAdr >= 0) {
      this.rootQposAdr = this.model.jnt_qposadr[jntAdr];
      this.rootQvelAdr = this.model.jnt_dofadr[jntAdr];
      console.log(`Pelvis root joint: qpos_adr=${this.rootQposAdr}, qvel_adr=${this.rootQvelAdr}`);
    }
  }

  console.log('Body name map:', Object.keys(this.bodyNameToId));

  this.bodyNameToMocapId = {};
  if (this.model.body_mocapid) {
    for (const [bodyName, bodyId] of Object.entries(this.bodyNameToId)) {
      const mocapId = this.model.body_mocapid[bodyId];
      if (mocapId !== undefined && mocapId >= 0) {
        this.bodyNameToMocapId[bodyName] = mocapId;
      }
    }
  }
  console.log('Mocap body map:', this.bodyNameToMocapId);
}
...
export async function reloadPolicy(policy_path, options = {}) {
  ...
  // Dispose old runners' ONNX sessions before creating new one
  if (this.policyRunner?.dispose) {
    await this.policyRunner.dispose();
  }
  if (this.interactionRunner?.dispose) {
    await this.interactionRunner.dispose();
  ...
  if (config.policy_type === 'carrybox_bbox_wtac') {
    this.isInteractionMode = false;
    this.policyRunner = new CarryBoxWTACPolicyRunner(config, {
      bodyNameToId: this.bodyNameToId,
      readBodyPose: (bodyId) => {
        const pos = new THREE.Vector3();
        const quat = new Float32Array(4);
        getPosition(this.simulation.xpos, bodyId, pos, false);
        getMujocoQuaternion(this.simulation.xquat, bodyId, quat, 0);
        return {
          pos: new Float32Array([pos.x, pos.y, pos.z]),
          quat
        };
      }
    });
    ...
    if (boxBodyId !== undefined && this.policyRunner.refObjectPos?.[0] && this.policyRunner.refObjectQuat?.[0]) {
      ...
      if (jointNum > 0 && jointAdr >= 0) {
        ...
        this.simulation.qpos.set(this.policyRunner.refObjectPos[0], qadr);
        this.simulation.qpos.set(this.policyRunner.refObjectQuat[0], qadr + 3);
        this.simulation.qvel.fill(0, vadr, vadr + 6);
        this.simulation.forward();
        const resetState = this.readPolicyState?.();
        if (resetState) {
...
/**
 * Load and initialize an interaction policy (SDF-based, with latent encoder).
 * `this` is the MuJoCoDemo instance.
 */
export async function reloadInteractionPolicy(policy_path, preloadedConfig = null) {
  this.currentPolicyPath = policy_path;
  this.isInteractionMode = true;
  console.log('Loading interaction policy:', policy_path);

  // Wait for any running inference
  if (this.interactionRunner?.isInferencing) {
    await new Promise((resolve) => setTimeout(resolve, 50));
  }
  if (this.policyRunner?.isInferencing) {
    await new Promise((resolve) => setTimeout(resolve, 50));
  }

  const config = preloadedConfig ?? await (await fetch(policy_path)).json();

  const policyJointNames = config.policy_joint_names;
  if (!policyJointNames || policyJointNames.length === 0) {
    throw new Error('Interaction policy config must include policy_joint_names');
  }
  ...
  this.kdPolicy = toFloatArray(config.damping, this.numActions, 0.0);
  this.control_type = config.control_type ?? 'joint_position';
  this.defaultJposPolicy = toFloatArray(config.default_joint_pos, this.numActions, 0.0);

  // Resolve SDF body IDs
  this.sdfBodyIds = [];
  const sdfBodyNames = config.sdf?.query_body_names ?? [];
  for (const name of sdfBodyNames) {
    const id = this.bodyNameToId[name];
    if (id === undefined) {
      console.warn(`SDF query body "${name}" not found in MuJoCo model`);
      this.sdfBodyIds.push(0);
    } else {
      this.sdfBodyIds.push(id);
    }
  }

  // Resolve scene object body IDs
  this.sceneObjectBodyIds = {};
  const objectBodyNames = config.sdf?.scene_object_body_names ?? [];
  for (const name of objectBodyNames) {
    const id = this.bodyNameToId[name];
    if (id !== undefined) {
      this.sceneObjectBodyIds[name] = id;
    } else {
      console.warn(`Scene object body "${name}" not found in MuJoCo model`);
    }
  }

  // Dispose old runners' ONNX sessions before creating new ones
  if (this.interactionRunner?.dispose) {
    await this.interactionRunner.dispose();
  }
  if (this.policyRunner?.dispose) {
  ...
  // Create and initialize the interaction runner
  this.simulation.resetData();
  this.simulation.forward();

  this.interactionRunner = new InteractionPolicyRunner(config);
  await this.interactionRunner.init();
  this.policyRunner = null; // disable tracking runner
  ...
  for (let g = 0; g < model.ngeom; g++) {
    ...
    if (type == mujoco.mjtGeom.mjGEOM_PLANE.value) {
      ...
      if (!(meshID in meshes)) {
        ...
        // Copy vertex/normal data instead of modifying WASM memory in-place.
        // MuJoCo's collision system (convex hull) references mesh_vert at
        // runtime, so swizzling in-place corrupts collision for mesh geoms.
        let raw_verts = model.mesh_vert.subarray(
          model.mesh_vertadr[meshID] * 3,
          (model.mesh_vertadr[meshID] + model.mesh_vertnum[meshID]) * 3);
        let vertex_buffer = new Float32Array(raw_verts.length);
        for (let v = 0; v < raw_verts.length; v += 3) {
          vertex_buffer[v]     = raw_verts[v];
          vertex_buffer[v + 1] = raw_verts[v + 2];
          vertex_buffer[v + 2] = -raw_verts[v + 1];
        }

        let raw_normals = model.mesh_normal.subarray(
          model.mesh_vertadr[meshID] * 3,
          (model.mesh_vertadr[meshID] + model.mesh_vertnum[meshID]) * 3);
        let normal_buffer = new Float32Array(raw_normals.length);
        for (let v = 0; v < raw_normals.length; v += 3) {
          normal_buffer[v]     = raw_normals[v];
          normal_buffer[v + 1] = raw_normals[v + 2];
          normal_buffer[v + 2] = -raw_normals[v + 1];
        }
...
/**
 * Read raw MuJoCo quaternion (w,x,y,z) into a flat Float32Array.
 * No coordinate swizzle - returns values in MuJoCo physics frame.
 */
export function getMujocoQuaternion(buffer, index, outArray, offset) {
  outArray[offset]     = buffer[(index * 4) + 0]; // w
  outArray[offset + 1] = buffer[(index * 4) + 1]; // x
  outArray[offset + 2] = buffer[(index * 4) + 2]; // y
  outArray[offset + 3] = buffer[(index * 4) + 3]; // z
}
...
function createSimulationWrapper(mujoco, model, data) {
  ...
  return {
    ...
    get mocap_pos() { return data.mocap_pos; },
    get mocap_quat() { return data.mocap_quat; },
